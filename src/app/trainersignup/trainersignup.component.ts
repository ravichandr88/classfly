...
export class TrainersignupComponent implements OnInit {
  ...
  //variables for form boolean visibility
  form1:boolean = true
  form2:boolean=false
  form3:boolean=false
  ...
  //city code varible declaration starts here
  cityoptions : string[] = ['Bangalore','Mysore','Coorg']
  filteredOptions: Observable<string[]>;
  cityControl = new FormControl();
  //city code variable declaration ends here
  ...
  private _sfilter(value: string): string[] {
    const sfilterValue = value.toLowerCase();
    this.myForm.patchValue({
      state: value, 
      // formControlName2: myValue2 (can be omitted)
    });
    //function call to update the city list for auto complete view
    this.cities(this.stateoptions.indexOf(value));
    return this.stateoptions.filter(option => option.toLowerCase().includes(sfilterValue));
  }

  //state code ends here

  // city selection code starts 
...
// city selection code ends here

//start of skills input required code
fruits:string[]=['Java','AutoCAD','MS-Word']
...
//end of skills input required code

//start of language input 

  langCtrl = new FormControl();
  fileteredLang: Observable<string[]>;
  langs: string[] = ['English'];
  allLangs: string[] = ["Hindi", "English" ,	"Bengali", "Marathi" ,"Telugu" ,	"Tamil" ,	"Gujarati" ,	"Urdu" 	,"Kannada" 	,"Odia" 	,"Malayalam" ,	"Punjabi" ,	"Sanskrit"];
  ...
  ladd(event: MatChipInputEvent): void {
    const input = event.input;
    const value = event.value;

    // Add our fruit
    if ((value || '').trim()) {
      this.langs.push(value.trim());
    }
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    this.langs.push(event.option.viewValue);
    this.langInput.nativeElement.value = '';
    this.langCtrl.setValue(null);
  }

  private lfilter(value: string): string[] {
    const filterValue = value.toLowerCase();

    return this.allLangs.filter(lang => lang.toLowerCase().indexOf(filterValue) === 0);
  }
//end language input code
  ...
  constructor(public http:HttpClient,private dialogref:MatDialogRef<TrainersignupComponent>,public dialog: MatDialog,private route: ActivatedRoute,private formBuilder : FormBuilder,private data : DataService, private router : Router) { }
  
   expRestrict(control: AbstractControl):{[key:string]:boolean}|null
  {
  ...
  ngOnInit() {
    ...
    this.fileteredLang = this.langCtrl.valueChanges.pipe(
      startWith(null),
      map((lang: string | null) => lang ? this.lfilter(lang) : this.allLangs.slice()));
   ...
   // }
    ...
    this.myForm = this.formBuilder.group({
      ...
      dscp:['',[Validators.required,this.dscpvalidate]],
      techkeys:[this.fruits,Validators.required],
      langs:[this.langs,Validators.required],
...
this.eduForm = this.formBuilder.group({
  ...
  experience:['',[Validators.required,this.expRestrict]],
  level:[2],
  session_id:['',Validators.required]
})
  ...
  //valiadting the education form
  check()
  {
    console.log(this.eduForm)
    if (this.eduForm.valid)
    {
console.log(this.eduForm.getRawValue())
    }
    
    this.eduForm.patchValue({'session_id':sessionStorage.getItem('user')})
    
    var response
    this.data.trainersignup(this.eduForm.getRawValue()).subscribe(
      data => response = data,
      (err) => {},
      () => {
...
//function to validate the linkedin url
linkvalidate(control: AbstractControl):{[key:string]:boolean}|null
{
if( /(ftp|http|https):\/\/?(?:www\.)?linkedin.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/.test(control.value) || (control.value != undefined && !(isNaN(control.value))) )
{
   return null
}
...
dscpvalidate(control: AbstractControl):{[key:string]:boolean}|null
{

  if(control.value.length > 500  && (control.value !== undefined && (isNaN(control.value))))
  {
    return {'dscpvalidate':true}
  }
return null;
}
//end description validataion code

print()
{
  console.log(this.myForm)
  
  this.myForm.patchValue({'session_id':sessionStorage.getItem('user')})
  ...
  this.data.trainersignup(this.myForm.getRawValue()).subscribe(
    data => response = data,
    (err) => {},
    () => {
      if (response.code == 200)
      {
        //activate the second form 
        this.form1=false
        this.form2 = true
      }
      else
...
// console.log(s_a[1].split("|"));

if(id != -1)
{
  // console.log(id)
this.cityoptions =  s_a[id+1].split("|")
this.filteredOptions = this.cityControl.valueChanges.pipe(
  startWith(''),
    map(value => this._filter(value))
);
}
return ['Bangalore','Mysore','Mandya']
...
// code to work with image upload
checked = false;
  indeterminate = false;
  ...
  disabled = false;
    ...
    imgvsbl:boolean = true

    //code to select one of the avattar pictures selcted
    changeimg(url)
    {
      this.imgURL = 'assets/dpicon/'+url
    }
...
//end of code to work with image upload
...
fileChange(event) {
  ...
  if(event.target.files.length > 0) {
    ...
    var name = file.name
      ...
      //now ge the get upload url from backed using filename
      var response
      this.data.get_imgurl(name).subscribe(
        data => response = data,
        (err)=>{
 ...
 xhr.send(file);
      ...
      //if the user has any error messages from server
      else
      {
        alert(response['message'])
      }
  ...
  //function to move to next account details after profile update
